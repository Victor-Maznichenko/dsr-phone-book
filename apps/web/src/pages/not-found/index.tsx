import { Button, Container, Image, SimpleGrid, Text, Title } from '@mantine/core';
import styles from './styles.module.scss';

export const NotFoundPage = () => (
  <Container className={styles.root}>
    <SimpleGrid spacing={{ base: 40, sm: 80 }} cols={{ base: 1, sm: 2 }}>
      <Image src="/images/not-found.svg" className={styles.image} />
      <div>
        <Title className={styles.title}>Something is not right...</Title>
        <Text className={styles.description} c="dimmed" size="lg">
          Page you are trying to open does not exist. You may have mistyped the address, or the page has been moved to
          another URL. If you think this is an error contact support.
        </Text>
        <Button className={styles.button} variant="outline" size="md" mt="xl">
          Get back to home page
        </Button>
      </div>
    </SimpleGrid>
  </Container>
);
